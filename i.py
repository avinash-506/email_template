...
df = df.loc[df['SFAccountName'] == 'Media.net']

total_issues = df['Missing'].sum()
no_of_issues = df['Fixed'].sum()
no_of_fixed = df['New_missing'].sum()
with tempfile.NamedTemporaryFile(delete=True, suffix='.csv') as temp_csv:
    df.to_csv('medianet.csv')
...
with open('medianet.csv', "rb") as attachment:
